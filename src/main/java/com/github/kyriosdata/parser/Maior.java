...
/**
 * Expressão que verifica se uma expressão é maior que outra.
 */
class Maior implements Expressao {
     ...
     * forem {@code null}.
     */
    public Maior(Expressao p1, Expressao p2) {
        if (p1 == null || p2 == null) {
    ...
    @Override
    public float valor() {
        return maiorQue(parcelaUm.valor(), parcelaDois.valor()) ? 1f : 0f;
    }

    private boolean maiorQue(float f1, float f2) {

        return (f1 - f2) > 0.00001d;
    }
